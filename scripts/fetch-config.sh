...
if [ -z "$CONFIG_REPO_URL" ]; then
    echo "ERROR: CONFIG_REPO_URL 환경 변수가 설정되지 않았습니다."
    exit 1
fi
...
if [ -f "$TEMP_DIR/.env.$ENVIRONMENT" ]; then
    echo "SUCCESS: .env.$ENVIRONMENT 파일을 복사합니다."
    cp "$TEMP_DIR/.env.$ENVIRONMENT" "$CONFIG_DIR/.env.$ENVIRONMENT"
else
    echo "WARNING: .env.$ENVIRONMENT 파일이 없습니다. 기본 .env 파일을 사용합니다."
    if [ -f "$TEMP_DIR/.env" ]; then
        cp "$TEMP_DIR/.env" "$CONFIG_DIR/.env.$ENVIRONMENT"
    else
        echo "ERROR: 기본 .env 파일도 없습니다."
        exit 1
    fi
fi
...
if [ -d "$TEMP_DIR/$ENVIRONMENT" ]; then
    echo "SUCCESS: $ENVIRONMENT 환경별 설정을 복사합니다."
...
echo "SUCCESS: Config 파일 동기화가 완료되었습니다!"
echo "복사된 파일 목록:"
ls -la "$CONFIG_DIR"

# 환경 변수 파일 검증
if [ -f "$CONFIG_DIR/.env.$ENVIRONMENT" ]; then
    echo "SUCCESS: 환경 설정 파일 검증 완료: .env.$ENVIRONMENT"
    # 민감한 정보 제외하고 설정 확인
    echo "주요 설정 변수:"
    grep -E "^(ENVIRONMENT|DEBUG|LOG_LEVEL|SPRING_BOOT_BASE_URL)" "$CONFIG_DIR/.env.$ENVIRONMENT" | head -5 || true
else
    echo "ERROR: 환경 설정 파일이 없습니다: .env.$ENVIRONMENT"
    exit 1
fi

echo "SUCCESS: Config 동기화가 성공적으로 완료되었습니다!"
